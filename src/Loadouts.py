...
class Loadouts:
    ...
    def get_match_loadouts(
        self, match_id, players, weaponChoose, valoApiSkins, names, state="game"
    ):
    ...
    def convertLoadoutToJsonArray(self, PlayerInventorys, players, state, names):
        ...
        if state == "game":
            ...
            for i in range(len(PlayerInventorys)):
                ...
                spray_selections = [
                    s
                    for s in PlayerInventory.get("Expressions", {}).get(
                        "AESSelections", []
                    )
                        ...
                        for var_socket in sockets:
                            if socket == sockets[var_socket]:
                                final_json[players[i]["Subject"]]["Weapons"][
                                    ...
                                    {
                                        var_socket: PlayerInventory["Items"][skin][
                                            "Sockets"
                                        ][socket]["Item"]["ID"]
                                    }
                                )
                    ...
                    for socket in PlayerInventory["Items"][skin]["Sockets"]:
                        if sockets["skin_buddy"] == socket:
                            for buddy in valoApiBuddies.json()["data"]:
                                if (
                                    buddy["uuid"]
                                    == PlayerInventory["Items"][skin]["Sockets"][
                                        socket
                                    ]["Item"]["ID"]
                                ):
                                    final_json[players[i]["Subject"]]["Weapons"][
